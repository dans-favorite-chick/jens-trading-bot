    ...
    *,
    ...
    try:
        ...
        try:
            ...
            if kind == "trades":
                rows_inserted = _ingest_trades(con, csv_path, run_id, header)
...
def _ingest_trades(
    ...
    present = set(header) & SOURCE_COLS
    csv_str = _safe_csv_path(csv_path)

    def col(name, *, fallback=None):
        return _col_or_null(name, present, fallback=fallback)

    entry_context_expr = (
        "TRY_CAST(entry_context AS JSON)"
        if "entry_context" in present
        else "NULL"
    )
